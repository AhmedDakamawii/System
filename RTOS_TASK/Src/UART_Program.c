 ...
 *
 ...
 void UART_ReciveString(){
 		...
 		while ( LCDRecieve != '#') {
 			...
 			LCDRecieve = USART_ReceiveByte(UART3) ;

 			if ((LCDRecieve >= 'A' && LCDRecieve <= 'Z')
 				|| (LCDRecieve >= 'a' && LCDRecieve <= 'z' || LCDRecieve== ' ') ){

 //				USART_TransmitString(UART3,&LCDRecieve);
 				LCD_VidSendChar(LCDRecieve);
